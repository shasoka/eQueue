...
from core.schemas.workspace import WorkspaceJoin, WorkspaceCreate, \
    WorkspaceUpdate
...
async def get_workspace_by_id_without_selectinload(
		workspace_id: int,
		session: AsyncSession,
) -> Workspace | None:
...
async def get_workspace_subjects_casted(
	...
	workspace = result.first()
	return [
		WorkspaceSubjectRead.model_validate(subj.dict()) for subj in
		workspace.subjects
	]
...
async def create_workspace(
	...
	if await get_group(session, workspace_in.group_id):
		if not await get_workspace_by_constraint(
			...
			workspace = Workspace(
				**workspace_in.model_dump(exclude_unset=True))
...
async def update_workspace(
	...
	if workspace := await get_workspace_by_id(
		...
		if not await check_workspace_constraint(
			...
			for key, value in workspace_upd.model_dump(
...
async def workspace_safe_delete(session: AsyncSession,
